<template>
  <div class="content tab-container" :class="{'iphonex-tab-container':isIphoneX,'iphonex-loadmores':isIphoneX}">
    <ul class="ul_content ul-content2" ref="tabelTiTle" v-show="!isNoData && fixedHeaderShow">
      ...
      <loadmore ref="myloadmore" class="loadmores" @loadTop='loadTop' :allLoaded='allLoaded'  :isNoData='isNoData' :maxDistance="90">
  ...
  export default {
    data(){
      return{
        ...
        dataListHeader:[],
        dataList:{},
        subjectId:JSON.parse(localStorage.getItem('rankDetailsDataId'))[0],
      ...
      genData(){ //获取全国门店列表
        ...
        .then(res => {
          ...
          if (res.data.code == 200) {
            ...
            //判断排名是否在首屏
            if(document.getElementById('current').offsetTop-windowHeight<0 && that.dataList.rank !=null && that.dataList.rank !=''){
              that.rankingStatus = true;
            }else{
              that.rankingStatus = false;
            }
          }
